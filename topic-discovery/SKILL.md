...
**执行规则：**
- ✅ **Step 0 必须提问**（使用 AskUserQuestion 工具收集必要信息）
- ❌ **Step 1-5 禁止中途停顿**（静默执行，不请求确认）
- ❌ **禁止** 询问"是否需要更多信息"
- ❌ **禁止** 中途停顿等待用户反馈
...
**⚠️ 重要说明**：
- Step 0 的提问是收集执行参数，不是"中途停顿"
- Step 1-5 执行过程中不得再次询问用户
...
| 选题标题 | 具体可执行的选题方向 |
| 选题角度 | 切入点和差异化描述 |
| **用户痛点** | **核心问题 + 困扰原因 + 解决方案（v4.1新增，必填）** |
| **互动引导策略** | **内容设计 + 引导话术 + 预期互动 + 回复策略（v4.1新增，必填）** |
| 目标人群 | 受众画像 |
| 内容形式 | 图文 / 短视频 / 长视频 |
| 推荐平台 | 带权重的平台标签 |
| 爆款潜力 | 1-5星评分 |
