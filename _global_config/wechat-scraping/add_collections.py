...
for article in articles:
    ...
    if title in collection_mappings:
        ...
        for pattern, collection_info in collection_mappings.items():
            ...
            # No collection found
            article['collection_name'] = None
            article['collection_type'] = None

# Save updated data
with open('wechat_articles_data_with_collections.json', 'w', encoding='utf-8') as f:
    json.dump(articles, f, ensure_ascii=False, indent=2)

print(f'Updated {updated_count} articles with collection information')
print(f'\nMatched articles:')
for title in matched_articles:
    print(f'  - {title}')

# Print articles without collections
articles_without_collections = [a['title'] for a in articles if a['collection_name'] is None]
print(f'\n{len(articles_without_collections)} articles without collections:')
for title in articles_without_collections:
    print(f'  - {title}')
